...
export async function generateStaticParams() {
  return getContentSlugs("ai/use-cases").map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  try {
    const meta = await getContentMetadata("ai/use-cases", slug);
    return {
      title: meta.title,
      description: meta.description,
      alternates: {
        canonical: `/ai/use-cases/${slug}`,
      },
      openGraph: {
        type: "article",
        title: meta.title,
...
export default async function UseCasePage({
  ...
  let mod;
  try {
    mod = await import(`@/content/ai/use-cases/${slug}.mdx`);
  } catch {
    notFound();
  }

  const { metadata, default: Content } = mod;
  const meta = metadata as UseCaseMetadata;
    ...
    <>
      <BreadcrumbSchema
        items={[
          ...
          { name: meta.title, href: `/ai/use-cases/${slug}` },
        ]}
      />
      <ArticleSchema
        title={meta.title}
      ...
      <GridWrapper>
        <GridCardSection noBottomGap className="relative overflow-hidden">
          <AbstractAsciiBackground seed={`usecase-${slug}`} />
